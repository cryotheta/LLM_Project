...
for epoch in tqdm(range(epochs)):
    ...
    for batch in train_dataloader:
        ...
        if counter%100==0:
            loss_monitor.append(loss.detach().cpu().item())
...
# Save the trained model
    model.save_pretrained(f"./toxic_model_multitarget_epoch_{epoch}")
    tokenizer.save_pretrained(f"./toxic_model_multitarget_epoch_{epoch}")
